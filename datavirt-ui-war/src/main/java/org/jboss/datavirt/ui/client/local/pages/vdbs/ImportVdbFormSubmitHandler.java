...
import org.jboss.datavirt.ui.server.servlets.DataVirtUploadServlet;
...
/**
 * The form submit handler used by the {@link ImportVdbDialog}.
 *
 * @author mdrillin@redhat.com
 */
@Dependent
public class ImportVdbFormSubmitHandler implements SubmitHandler, SubmitCompleteHandler {
     ...
     * Constructor.
     */
    public ImportVdbFormSubmitHandler() {
    }
    ...
    public void onSubmit(SubmitEvent event) {
        ...
        notification = notificationService.startProgressNotification(
                i18n.format("import-vdb-submit.uploading.title"), //$NON-NLS-1$
                i18n.format("import-vdb-submit.uploading.msg")); //$NON-NLS-1$
    }
        ...
        if (results.isError()) {
            if (results.getError() != null) {
                notificationService.completeProgressNotification(
                        ...
                        i18n.format("import-vdb-submit.upload-error.title"), //$NON-NLS-1$
                        results.getError());
            } else {
                notificationService.completeProgressNotification(
                        notification.getUuid(),
                        i18n.format("import-vdb-submit.upload-error.title"), //$NON-NLS-1$
                        i18n.format("import-vdb-submit.upload-error.msg")); //$NON-NLS-1$
            }
        } else {
