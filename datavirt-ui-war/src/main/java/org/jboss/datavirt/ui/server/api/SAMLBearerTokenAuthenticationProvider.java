...
public class SAMLBearerTokenAuthenticationProvider implements AuthenticationProvider {
    ...
    private static String createSAMLBearerTokenAssertion() {
        String issuer = DataVirtUIConfig.config.getString(DataVirtUIConfig.SRAMP_API_SAML_AUTH_ISSUER);
        String service = DataVirtUIConfig.config.getString(DataVirtUIConfig.SRAMP_API_SAML_AUTH_SERVICE);
        String samlAssertion = Jboss7SAMLBearerTokenUtil.createSAMLAssertion(issuer, service);
        boolean signAssertion = "true".equals(DataVirtUIConfig.config.getString(DataVirtUIConfig.SRAMP_API_SAML_AUTH_SIGN_ASSERTIONS)); //$NON-NLS-1$
        if (signAssertion) {
            String keystorePath = DataVirtUIConfig.config.getString(DataVirtUIConfig.SRAMP_API_SAML_AUTH_KEYSTORE);
            String keystorePassword = DataVirtUIConfig.config.getString(DataVirtUIConfig.SRAMP_API_SAML_AUTH_KEYSTORE_PASSWORD);
            String keyAlias = DataVirtUIConfig.config.getString(DataVirtUIConfig.SRAMP_API_SAML_AUTH_KEY_ALIAS);
            String keyAliasPassword = DataVirtUIConfig.config.getString(DataVirtUIConfig.SRAMP_API_SAML_AUTH_KEY_PASSWORD);
//            try {
