 ...
 */
package org.jboss.datavirt.ui.server.filters;
 ...
 *
 * Additionally, the locale is made available to the s-ramp client accessor so that it can
 * properly configure the client with the current locale.
 *
...
public class LocaleFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        AdminApiClientAccessor.setLocale(locale);
        chain.doFilter(request, response);
        AbstractMessages.clearLocale();
        AdminApiClientAccessor.clearLocale();
    }
